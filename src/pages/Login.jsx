...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
       ...
       try {
        ...
        if (state === 'Sign Up') {
            ...
            const {data} = await axois.post(backendUrl + '/api/user/register',{name, password, email})
            if (data.success) {
          ...
          const {data} = await axois.post(backendUrl + '/api/user/login',{password, email})
            if (data.success) {
              localStorage.setItem('token',data.token)
              setToken(data.token)
            } else {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border border-amber-50 rounded-xl text-zinc-600 text-sm shadow-lg'>
